...
import { FiStar, FiUploadCloud, FiChevronDown, FiChevronUp, FiImage } from 'react-icons/fi';
...
const SuccessStoriesAdmin = () => {
  const CMS_API = API.CMS_SUCCESS_STORIES;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      resetForm();
      fetchItems();
      notifySuccess("Success Story synchronized successfully!");
    } catch (err) {
  ...
  const handleDelete = async (e, id) => {
    e.stopPropagation();
    if (window.confirm("Delete this Success Story permanently?")) {
      try {
  ...
  const resetForm = () => {
    setFormData({ id: null, name: '', role: '', image: '', quote: '', order: 0 });
    setIsEditing(false);
    setShowForm(false);
  };
    ...
    <div className="min-h-screen bg-[#F9FAFB] pb-20 font-sans text-[#292c44]">
      <nav className="sticky top-0 z-30 bg-white/90 backdrop-blur-md border-b border-gray-100 shadow-sm">
        <div className="max-w-3xl mx-auto px-6 h-20 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-bold">Career - Success Stories</h1>
          </div>
          <button
            ...
            >
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-black text-[#292c44]">
                  {isEditing ? '⚡ Update Story' : '🌟 New Success Story'}
                </h2>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <input className="px-5 py-4 bg-gray-50 border rounded-2xl outline-none focus:ring-2 focus:ring-[#292c44]" placeholder="Person's Name" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} required />
                  <input className="px-5 py-4 bg-gray-50 border rounded-2xl outline-none focus:ring-2 focus:ring-[#292c44]" placeholder="Role" value={formData.role} onChange={(e) => setFormData({...formData, role: e.target.value})} required />
                ...
                <label className={`flex items-center justify-between px-5 py-4 bg-gray-50 border rounded-2xl cursor-pointer ${formData.image ? 'text-green-600 border-green-200' : 'text-gray-400'}`}>
                  ...
                  <span className="truncate">{uploading ? "Uploading..." : formData.image ? "Image Ready" : "Upload Photo to S3"}</span>
                  <FiUploadCloud />
                </label>
                ...
                <textarea rows="3" className="w-full px-5 py-4 bg-gray-50 border rounded-2xl outline-none" placeholder="Their Quote" value={formData.quote} onChange={(e) => setFormData({...formData, quote: e.target.value})} required />
                <button type="submit" disabled={loading || uploading} className="w-full py-4 bg-[#292c44] text-white rounded-2xl font-bold shadow-lg disabled:bg-gray-300">
                  {loading ? "Saving..." : (isEditing ? "Update Story" : "Add Story")}
                </button>
              </form>
            </motion.div>
          </motion.div>
      ...
      <main className="max-w-3xl mx-auto px-6 mt-10">
        ...
        <div className="space-y-6">
          <h3 className="text-[10px] font-black text-gray-300 uppercase tracking-widest ml-2">Stories Inventory</h3>
            ...
            <div key={item._id} onClick={() => setExpandedId(expandedId === item._id ? null : item._id)} className="bg-white border border-gray-100 rounded-[2.5rem] p-6 cursor-pointer hover:shadow-lg transition-all">
               <div className="flex gap-4 items-center">
                  <div className="w-12 h-12 bg-gray-900 rounded-xl flex items-center justify-center text-white relative overflow-hidden shrink-0">
                    ...
                    {item.image && <img src={item.image} className="absolute inset-0 w-full h-full object-cover opacity-60" alt="" />}
                  </div>
               ...
               <AnimatePresence>
                 {expandedId === item._id && (
                   <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="mt-6 pt-6 border-t border-gray-50 overflow-hidden">
                      <p className="text-sm font-semibold text-gray-700 mb-1 whitespace-pre-wrap break-words">
                        {item.role}
                      </p>
                      <p className="text-sm text-gray-500 mb-6 leading-relaxed italic whitespace-pre-wrap break-words">
                        "{item.quote}"
                      </p>
