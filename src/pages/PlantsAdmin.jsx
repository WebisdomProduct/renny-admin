...
import { FiMapPin, FiUploadCloud, FiChevronDown, FiChevronUp, FiImage } from 'react-icons/fi';
...
const PlantsAdmin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      resetForm();
      fetchItems();
      notifySuccess("Plant entry synchronized successfully!");
    } catch (err) {
  ...
  const handleDelete = async (e, id) => {
    e.stopPropagation();
    if (window.confirm("Delete this Plant entry permanently?")) {
      try {
    ...
    <div className="min-h-screen bg-[#F9FAFB] pb-20 font-sans text-[#292c44]">
      <nav className="sticky top-0 z-30 bg-white/90 backdrop-blur-md border-b border-gray-100 shadow-sm">
        <div className="max-w-3xl mx-auto px-6 h-20 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-bold">Contact Us - Plants</h1>
          </div>
          <button
            ...
            >
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-black text-[#292c44]">
                  {isEditing ? '⚡ Update Plant' : '🏭 New Plant Location'}
                </h2>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <label className={`flex items-center justify-between px-5 py-4 bg-gray-50 border rounded-2xl cursor-pointer ${formData.image ? 'text-green-600 border-green-200' : 'text-gray-400'}`}>
                  ...
                  <span className="truncate">{uploading ? "Uploading..." : formData.image ? "Image Ready" : "Upload Image to S3"}</span>
                  <FiUploadCloud />
                </label>
      ...
      <main className="max-w-3xl mx-auto px-6 mt-10">
        ...
        <div className="space-y-6">
          <h3 className="text-[10px] font-black text-gray-300 uppercase tracking-widest ml-2">Plants Inventory</h3>
          {items.map((item) => (
               ...
               <AnimatePresence>
                 {expandedId === item._id && (
                   <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="mt-6 pt-6 border-t border-gray-50 overflow-hidden">
                      {item.image && <img src={item.image} alt="Plant" className="w-full rounded-2xl bg-gray-100 aspect-video object-cover mb-4 shadow-inner" />}
                      <div className="text-sm text-gray-500 mb-6 leading-relaxed">
                        <p className="font-bold text-gray-700">Address:</p>
                        <p className="mb-3 whitespace-pre-wrap">{item.address}</p>
                        <p className="font-bold text-gray-700">Order:</p>
                        <p className="whitespace-pre-wrap">{item.order || 0}</p>
                      </div>
